...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div style={{
      ...
      padding:"10px 14px", borderRadius:4, fontSize:"0.75rem"
    }}>
      <div style={{color:"var(--muted)", marginBottom:4}}>Run ID: {d.run_id?.slice(0,16)}…</div>
      <div>Status: <span style={{color: STATUS_COLOR[d.status] ?? "var(--muted)"}}>{d.status}</span></div>
      <div>Duration: <span style={{color:"var(--text)"}}>{d.execution_time}s</span></div>
    </div>
  );
};
...
export default function JobRunsPanel({ jobName, onClose }) {
  ...
  const avgTime   = runs.length
    ? Math.round(runs.reduce((a, r) => a + (r.execution_time || 0), 0) / runs.length)
    : 0;
    ...
    <div className="panel">
      ...
      {!loading && !error && (
        <div className="runs-meta">
          ...
          <div className="meta-item">
            ...
            <span className="meta-value" style={{color:"var(--accent)"}}>{succeeded}</span>
          </div>
          <div className="meta-item">
            <span className="meta-label">Failed</span>
            <span className="meta-value" style={{color:"var(--danger)"}}>{failed}</span>
          </div>
          <div className="meta-item">
      ...
      {!loading && !error && runs.length > 0 && (
        <div style={{padding:"1.25rem", borderBottom:"1px solid var(--border)"}}>
          <div style={{fontSize:"0.65rem", color:"var(--muted)", marginBottom:"0.75rem", letterSpacing:"0.08em", textTransform:"uppercase"}}>
          ...
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={[...runs].reverse()} barSize={18}>
              ...
              <Bar dataKey="execution_time" radius={[3,3,0,0]}>
                {runs.map((r, i) => (
                  <Cell key={i} fill={STATUS_COLOR[r.status] ?? "var(--muted)"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
            ...
            <tbody>
              {runs.map((run) => (
                <tr key={run.run_id}>
                  ...
                  <td>{run.started_on ? new Date(run.started_on).toLocaleString() : "—"}</td>
                  <td>{run.execution_time ?? "—"}s</td>
                  <td>{run.dpu_seconds ?? "—"}</td>
                  <td style={{color:"var(--danger)", fontSize:"0.72rem", maxWidth:200, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>
                    {run.error_message ?? "—"}
                  </td>
                </tr>
              ))}
